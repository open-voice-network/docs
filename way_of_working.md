...
To support an inclusive ratification of ideas, we use architecture decision tool `adr-tools`.
To record important architecture decision, we use [adr-tools](https://github.com/npryce/adr-tools). To install, type the following in the terminal command. 

### Installing `adr-tools`

On MacOS, use `brew` to install the ADR tools:

```sh
brew install adr-tools
```

For other operating systems, see [the install instructions](https://github.com/npryce/adr-tools/blob/master/INSTALL.md).
